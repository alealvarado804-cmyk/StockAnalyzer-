...
function computeSqueezeFullSeries(data, bbLen = 20, kcLen = 20, kcMult = 1.5, momLen = 12) {
  if (!data || data.length < Math.max(bbLen, kcLen) + momLen) return (data || []).map(() => ({
    sqzOn: false,
    sqzOff: false,
    noSqz: true,
    momVal: null
  ...
  for (let i = 0; i < n; i++) {
    ...
    const bbU = bbM + 2 * bbSd,
      bbL = bbM - 2 * bbSd;
    // Keltner Channel
    let trS = 0;
    for (let j = i - kcLen + 1; j <= i; j++) {
      const pC = j > 0 ? closes[j - 1] : closes[j];
      trS += Math.max(highs[j] - lows[j], Math.abs(highs[j] - pC), Math.abs(lows[j] - pC));
    }
    const kcA = trS / kcLen;
    const kcU = smaKC[i] + kcMult * kcA,
      kcL = smaKC[i] - kcMult * kcA;
    const sqzOn = bbU <= kcU && bbL >= kcL,
      sqzOff = bbU >= kcU && bbL <= kcL;
...
function computeADXFullSeries(data, period = 14) {
  ...
  for (let i = 1; i < n; i++) {
    ...
    const up = hi - pH,
      dn = pL - lo;
    pDM.push(up > 0 && up > dn ? up : 0);
    mDM.push(dn > 0 && dn > up ? dn : 0);
  }
  const ws = (arr, p) => {
    const r = [];
  ...
  for (let i = 0; i < trS.length; i++) {
    const p = trS[i] > 0 ? 100 * pS[i] / trS[i] : 0,
      m = trS[i] > 0 ? 100 * mS[i] / trS[i] : 0;
    pDI.push(p);
    mDI.push(m);
  ...
  for (let j = 0; j < adxS.length; j++) {
    ...
    if (idx < n) result[idx] = {
      adx: adxS[j],
      plusDI: pDI[j + period - 1] ?? null,
      minusDI: mDI[j + period - 1] ?? null
    };
  }
  return result;
}
...
function computeVolumeProfile(data, numBuckets = 24) {
  ...
  let vol = buckets[pocIdx],
    vaH = pocIdx,
    vaL = pocIdx;
  while (vol < total * 0.70) {
    const aH = vaH < numBuckets - 1 ? buckets[vaH + 1] : 0,
      aL = vaL > 0 ? buckets[vaL - 1] : 0;
    if (!aH && !aL) break;
    if (aH >= aL && vaH < numBuckets - 1) {
      vaH++;
      vol += aH;
    } else if (vaL > 0) {
      vaL--;
...
function detectSqueezeDivergences(closes, sqzSeries) {
  ...
  for (let i = LB; i < n - LB; i++) {
    ...
    if (ph) pH.push(i);
    if (pl) pL.push(i);
    if (q != null && sh) sH.push(i);
    if (q != null && sl) sL.push(i);
  }
  const PROX = 6,
    divs = [];
  for (let i = 1; i < pH.length; i++) {
    const i1 = pH[i - 1],
      i2 = pH[i];
    if (i2 - i1 < 5) continue;
    const q1 = sH.find(s => Math.abs(s - i1) <= PROX),
      q2 = sH.find(s => Math.abs(s - i2) <= PROX);
    if (q1 == null || q2 == null) continue;
    if (closes[i2] > closes[i1] && sv[q2] != null && sv[q1] != null && sv[q2] < sv[q1] * 0.93) divs.push({
      type: 'bearish',
      idx1: i1,
      idx2: i2,
      price1: closes[i1],
  ...
  for (let i = 1; i < pL.length; i++) {
    const i1 = pL[i - 1],
      i2 = pL[i];
    if (i2 - i1 < 5) continue;
    const q1 = sL.find(s => Math.abs(s - i1) <= PROX),
      q2 = sL.find(s => Math.abs(s - i2) <= PROX);
    if (q1 == null || q2 == null) continue;
    if (closes[i2] < closes[i1] && sv[q2] != null && sv[q1] != null && sv[q2] > sv[q1] * 1.07) divs.push({
      type: 'bullish',
      idx1: i1,
      idx2: i2,
      price1: closes[i1],
...
function rateSensitivityPenalty(netDebtEbitda, interestCov, quadrant) {
  ...
  const nd = Number(netDebtEbitda),
    ic = Number(interestCov);
  let pen = 0;
  if (isFinite(nd) && nd > 3) pen += 4;
  if (isFinite(ic) && ic > 0 && ic < 3) pen += 4;else if (isFinite(ic) && ic >= 3 && ic < 5) pen += 2;
...
function ipoEventTag(profile) {
  ...
  if (days >= 90 && days <= 200) return {
    label: "Ventana de lockup",
    color: "#eb6459",
    note: "Lockup típico 90–180d post-IPO: posible shock de oferta (vender la noticia)."
  };
  if (days < 90) return {
    label: "Recién IPO",
    color: "#eca851",
    note: "Salió a bolsa hace <90d: histórico limitado, alta volatilidad."
  };
  return {
    label: "IPO <1 año",
    color: "#eca851",
    note: "IPO reciente: riesgo de rotación/lockup residual."
  };
}
...
function PriceChart({
  ...
  useEffect(() => {
    ...
    const onWheel = e => {
      ...
      if (ns >= filtered.length - 1) {
        setZoom(null);
        return;
      }
      let st = Math.round(anchor - frac * ns),
        en = st + ns;
      if (st < 0) {
        st = 0;
        en = ns;
      }
      if (en > filtered.length - 1) {
        en = filtered.length - 1;
        st = Math.max(0, en - ns);
      }
      setZoom({
        startIdx: st,
  ...
  const toggleFred = useCallback(async () => {
    ...
    if (_fredCache.FEDFUNDS) {
      setFredObs(_fredCache.FEDFUNDS);
      return;
    }
    ...
    try {
      ...
      const obs = (data?.observations || []).filter(o => o && o.value !== '.' && o.value != null).map(o => ({
        date: o.date,
  ...
  if (!view.length || view.length < 2) return /*#__PURE__*/React.createElement("div", {
    style: {
      height: 200,
      display: 'flex',
      alignItems: 'center',
  ...
  const adxPts = (() => {
    ...
    const seg = [];
    adxV.forEach((s, i) => {
      if (s?.adx != null) seg.push(`${px(i).toFixed(1)},${ay(s.adx).toFixed(1)}`);
    });
  ...
  const fredSamples = fredOn && fredObs && fredObs.length ? (() => {
    ...
    const valAt = t => {
      let v = null;
      for (const o of obsS) {
        if (new Date(o.date).getTime() <= t) v = o.val;else break;
      }
      return v ?? obsS[0].val;
    };
    return view.map((d, i) => ({
      i,
      val: valAt(new Date(d.date).getTime())
    }));
  })() : null;
  const fredVals = fredSamples ? fredSamples.map(s => s.val) : [];
  const fMin = fredVals.length ? Math.min(...fredVals) : 0,
    fMax = fredVals.length ? Math.max(...fredVals) : 1;
  ...
  const btnS = (on, c) => ({
    background: on ? c + '22' : '#1c1d26',
    border: `1px solid ${on ? c : '#24262f'}`,
    color: on ? c : '#787a83',
    padding: '2px 8px',
    borderRadius: 4,
    cursor: 'pointer',
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: btnS(emaOn, '#eca851')
  }, emaOn ? '●' : '○', " EMA"), emaOn && sorted.length < 55 && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 8,
      color: '#787a83',
  ...
  }, "EMA55 necesita \u226555 barras totales"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setSqzOn(v => !v),
    style: btnS(sqzOn, '#968ff7')
  }, sqzOn ? '●' : '○', " SQZ"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setAdxOn(v => !v),
    style: btnS(adxOn, '#edeef4')
  }, adxOn ? '●' : '○', " ADX"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setVpOn(v => !v),
    style: btnS(vpOn, '#5ac576')
  }, vpOn ? '●' : '○', " VOL PROFILE"), /*#__PURE__*/React.createElement("button", {
    onClick: toggleFred,
    style: btnS(fredOn, '#eca851')
    ...
    return /*#__PURE__*/React.createElement("g", null, /*#__PURE__*/React.createElement("rect", {
      ...
      width: cw,
      y: py(vMin + (vaH + 1) * bSz),
      height: Math.abs(py(vMin + vaL * bSz) - py(vMin + (vaH + 1) * bSz)),
      fill: "#968ff7",
      opacity: "0.05"
    }), buckets.map((v, i) => {
      const bw = v / maxVol * vpW;
      const bY = py(vMin + (i + 1) * bSz),
        bH = Math.max(1, Math.abs(py(vMin + i * bSz) - bY));
      ...
      return /*#__PURE__*/React.createElement("rect", {
        ...
        width: bw,
        height: bH,
        fill: isPOC ? '#eca851' : isVA ? '#968ff7' : '#33353f',
        opacity: isPOC ? 0.9 : isVA ? 0.6 : 0.45
      });
    }), /*#__PURE__*/React.createElement("line", {
      x1: pl,
      x2: pl + cw,
    ...
    strokeWidth: "1.8",
    strokeLinejoin: "round"
  }), ema55Pts && ema55Pts.map((seg, i) => /*#__PURE__*/React.createElement("polyline", {
    key: 'e55' + i,
    points: seg,
    fill: "none",
    stroke: "#c8844a",
    ...
    clipPath: "url(#cpPrice)",
    opacity: "0.9"
  })), ema10Pts && ema10Pts.map((seg, i) => /*#__PURE__*/React.createElement("polyline", {
    key: 'e10' + i,
    points: seg,
    fill: "none",
    stroke: "#6ea8ff",
    ...
    fill: "#787a83",
    textAnchor: "end"
  }, "52W L"), emaOn && /*#__PURE__*/React.createElement("g", null, /*#__PURE__*/React.createElement("line", {
    x1: W - 140,
    x2: W - 128,
    y1: pt + 10,
    y2: pt + 10,
    ...
    fontSize: "7.5",
    fill: "#c8844a"
  }, "EMA 55"), /*#__PURE__*/React.createElement("line", {
    x1: W - 78,
    x2: W - 66,
    y1: pt + 10,
    y2: pt + 10,
    ...
    return /*#__PURE__*/React.createElement("g", null, /*#__PURE__*/React.createElement("line", {
      ...
      fill: "#968ff7",
      fontWeight: "700"
    }, "SQUEEZE MOMENTUM"), sqzV.map((s, i) => {
      if (!s) return null;
      const cx = px(i);
      const dotFill = s.sqzOn ? '#787a83' : s.sqzOff ? '#edeef4' : '#2d2d3a';
      if (s.momVal == null) return /*#__PURE__*/React.createElement("circle", {
        key: i,
        cx: cx,
        cy: sqzL.top + 5,
        r: "1.5",
      ...
      const barTop = Math.min(sy(s.momVal), sqzL.mid),
        barH = Math.max(1, Math.abs(sy(s.momVal) - sqzL.mid));
      return /*#__PURE__*/React.createElement("g", {
        key: i
      }, /*#__PURE__*/React.createElement("rect", {
        x: cx - bw / 2,
    ...
    }), hx != null && /*#__PURE__*/React.createElement("line", {
      x1: hx,
      x2: hx,
      y1: sqzL.top,
      y2: sqzL.bot,
      ...
      fontSize: "7",
      fill: "#33353f"
    }, "No sqz"), /*#__PURE__*/React.createElement("circle", {
      cx: W - 80,
      cy: sqzL.top + 8,
      r: "3",
      fill: "#787a83"
      ...
      fontSize: "7",
      fill: "#787a83"
    }, "Loaded"), /*#__PURE__*/React.createElement("circle", {
      cx: W - 40,
      cy: sqzL.top + 8,
      r: "3",
      fill: "#edeef4"
    ...
    return /*#__PURE__*/React.createElement("g", null, /*#__PURE__*/React.createElement("line", {
      ...
      fill: "#edeef4",
      fontWeight: "700"
    }, "ADX  (trend strength)"), adxPts && /*#__PURE__*/React.createElement("polyline", {
      points: adxPts,
      fill: "none",
      stroke: "#edeef4",
      strokeWidth: "1.6",
      ...
      return /*#__PURE__*/React.createElement(React.Fragment, null, pPts.length > 1 && /*#__PURE__*/React.createElement("polyline", {
        points: pPts.join(' '),
        fill: "none",
        stroke: "#5ac576",
        strokeWidth: "0.8",
        opacity: "0.45",
        clipPath: "url(#cpAdx)"
      }), mPts.length > 1 && /*#__PURE__*/React.createElement("polyline", {
        points: mPts.join(' '),
        fill: "none",
        stroke: "#eb6459",
        strokeWidth: "0.8",
    ...
    fill: "#33353f",
    textAnchor: "middle"
  }, mLbls[m])), hx != null && /*#__PURE__*/React.createElement("g", null, /*#__PURE__*/React.createElement("line", {
    x1: hx,
    x2: hx,
    y1: pt,
    y2: priceBottom,
    ...
    stroke: "#15151c",
    strokeWidth: "1.5"
  }), hE10 != null && /*#__PURE__*/React.createElement("circle", {
    cx: hx,
    cy: py(hE10),
    r: "2.5",
    fill: "#6ea8ff",
    stroke: "#15151c",
    strokeWidth: "1"
  }), hE55 != null && /*#__PURE__*/React.createElement("circle", {
    cx: hx,
    cy: py(hE55),
    r: "2.5",
    fill: "#c8844a",
    stroke: "#15151c",
    strokeWidth: "1"
  }))), hd && hx != null && /*#__PURE__*/React.createElement("div", {
    style: {
      position: 'absolute',
      top: 28,
  ...
  }, "Vol: ", fmt.usd(hd.volume)), (hE10 != null || hE55 != null) && /*#__PURE__*/React.createElement("div", {
    style: {
      borderTop: '1px solid #24262f',
      marginTop: 5,
      paddingTop: 5
    }
  }, hE10 != null && /*#__PURE__*/React.createElement("div", {
    style: {
      color: '#6ea8ff',
      fontSize: 9
    }
  }, "EMA 10: $", hE10.toFixed(2)), hE55 != null && /*#__PURE__*/React.createElement("div", {
    style: {
      color: '#c8844a',
      fontSize: 9
    }
  }, "EMA 55: $", hE55.toFixed(2))), hAdx?.adx != null && /*#__PURE__*/React.createElement("div", {
    style: {
      borderTop: '1px solid #24262f',
      marginTop: 4,
      paddingTop: 4
  ...
  }, "ADX: ", hAdx.adx.toFixed(1), hAdx.adx > 23 ? ' ▲' : ' —'), hAdx.plusDI != null && /*#__PURE__*/React.createElement("div", {
    style: {
      color: '#5ac576',
      fontSize: 9
    }
  }, "+DI: ", hAdx.plusDI.toFixed(1)), hAdx.minusDI != null && /*#__PURE__*/React.createElement("div", {
    style: {
      color: '#eb6459',
      fontSize: 9
    }
  }, "-DI: ", hAdx.minusDI.toFixed(1))), hSqz?.momVal != null && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 3,
      color: hSqz.momVal > 0 ? '#5ac576' : '#eb6459',
      fontSize: 9
    }
...
function TLSignalPanel({
  ...
  const data = useMemo(() => {
    ...
    const sqzLast = sqzS[n - 1],
      sqzPrev = sqzS[n - 2];
    const adxS = computeADXFullSeries(sorted, 14);
    const adxLast = adxS[n - 1],
      adxPrev = adxS[n - 2];
    ...
    const emaLong = emaOk && e10 > e55,
      emaShort = emaOk && e10 < e55;
    ...
    const rH = sorted.slice(-20).map(d => d.high || d.close),
      rL = sorted.slice(-20).map(d => d.low || d.close);
    const swHigh = Math.max(...rH),
      swLow = Math.min(...rL);
    ...
    const riskL = (cur - stopL) / cur,
      riskS = (stopS - cur) / cur;
    const tp1L = cur + 2 * (cur - stopL),
      tp1S = cur - 2 * (stopS - cur);
    return {
      cur,
      e10,
      e55,
      ...
      adxStrong,
      adxActive,
      adxPDI: adxLast?.plusDI ?? null,
      adxMDI: adxLast?.minusDI ?? null,
      sqzMom,
      sqzLong,
      sqzShort,
      sqzFired,
  ...
  if (!fundamentalsLoaded) {
    ...
    gateMsg = 'Entrada SHORT — IC Score AVOID + 3/4 señales TL SHORT alineadas';
    gateColor = '#eb6459';
  } else if (lS >= 3 && icS <= 35) {
    gateMsg = 'CONFLICTO — señal TL LONG pero fundamentales AVOID';
    gateColor = '#eca851';
  } else if (sS >= 3 && icS >= 65) {
    gateMsg = 'CONFLICTO — señal TL SHORT pero fundamentales BUY';
    gateColor = '#eca851';
  } else if (lS >= 2 && icS >= 65) {
    gateMsg = 'En espera — fundamentales fuertes; aguardando confirmación técnica LONG';
    gateColor = '#968ff7';
  } else if (sS >= 2 && icS <= 35) {
    gateMsg = 'En espera — fundamentales débiles; señal técnica SHORT acumulándose';
    gateColor = '#968ff7';
  } else {
    gateMsg = 'Sin confluencia suficiente — mercado en consolidación o transición';
    gateColor = '#33353f';
  }
  const Pill = ({
    on,
    ...
    style: {
      background: on ? color + '18' : '#1c1d26',
      border: `1px solid ${on ? color : '#24262f'}`,
      borderRadius: 6,
      padding: '8px 11px',
      flex: '1 1 130px',
      opacity: on ? 1 : 0.5,
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "TL Confluence\u2122 Strategy"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 9,
      color: '#33353f',
      marginLeft: 10
    }
  }, "EMA 10/55  \xB7  Squeeze Momentum  \xB7  ADX  \xB7  Volume Profile")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 8
    }
  }, bias !== 'NEUTRAL' && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 10,
      color: '#33353f',
      fontFamily: 'Geist Mono,monospace'
    }
  }, scoreCount, "/4"), /*#__PURE__*/React.createElement("div", {
    style: {
  ...
  }, bias))), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 8,
      marginBottom: 12,
    ...
    on: emaLong || emaShort,
    color: emaLong ? '#5ac576' : emaShort ? '#eb6459' : '#787a83',
    ...
    on: sqzLong || sqzShort,
    color: sqzLong ? '#5ac576' : sqzShort ? '#eb6459' : '#787a83',
    ...
    on: vpAbove || vpBelow,
    color: vpAbove ? '#5ac576' : vpBelow ? '#eb6459' : '#787a83',
  ...
  })), divs.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 12,
      display: 'flex',
      flexWrap: 'wrap',
    ...
    }, d.type === 'bullish' ? '▲ Divergencia alcista' : '▼ Divergencia bajista', " \u2014 Squeeze vs precio", /*#__PURE__*/React.createElement("span", {
      style: {
        color: '#787a83',
        marginLeft: 6
      }
    }, "precio1 $", d.price1.toFixed(2), " \u2192 $", d.price2.toFixed(2)));
  })), isDirectional && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 8,
      marginBottom: 12,
    ...
    val: `$${stopL.toFixed(2)}`,
    color: '#eb6459',
    sub: `−${(riskL * 100).toFixed(1)}%  (bajo EMA55 + swing low)`
  } : {
    label: 'Stop Loss',
    val: `$${stopS.toFixed(2)}`,
    color: '#eb6459',
    sub: `+${(riskS * 100).toFixed(1)}%  (sobre EMA55 + swing high)`
  }, isLong ? {
    label: 'TP  1:2',
    val: `$${tp1L.toFixed(2)}`,
    color: '#5ac576',
    sub: `+${(riskL * 200).toFixed(1)}%`
  } : {
    label: 'TP  1:2',
    val: `$${tp1S.toFixed(2)}`,
    color: '#5ac576',
    sub: `−${(riskS * 200).toFixed(1)}%`
  }].map(({
    label,
    val,
    color,
  ...
  }, "IC Gate"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11,
      fontWeight: 700,
      color: icR.color,
  ...
  }, icS, "/100 ", icR.label), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 9,
      color: '#33353f'
    }
  ...
  }, scoreCount, "/4 TL ", scoreBias)), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1,
      fontSize: 11,
      color: gateColor,
  ...
  }, "\u2192 ", gateMsg)), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 8,
      color: '#33353f',
      marginTop: 8
    }
  }, "An\xE1lisis t\xE9cnico educativo \xB7 No constituye asesor\xEDa de inversi\xF3n"));
}
...
function InstitutionalHoldersPanel({
  ...
  }, pctFloat != null && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10,
      color: '#968ff7',
      fontFamily: 'Geist Mono,monospace',
  ...
  }, pctFloat.toFixed(1), "% float"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: biasColor,
      fontWeight: 600
...
function SectorRelStrengthCard({
  ...
  const impliedBeta = tR1Y != null && sR1Y != null && Math.abs(sR1Y) > 0.1 ? parseFloat((tR1Y / sR1Y).toFixed(2)) : null;
  return /*#__PURE__*/React.createElement("div", {
    style: {
      background: '#1c1d26',
      border: '1px solid #24262f',
  ...
  }, "Sector Relative Strength"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10,
      color: '#787a83'
    }
  }, ticker, " vs ", sectorEtf, " (", sector, ")")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
  ...
  }), ['Ticker', 'ETF', 'Alpha'].map(h => /*#__PURE__*/React.createElement("div", {
    key: h,
    style: {
      fontSize: 9,
      color: '#787a83',
  ...
  }, fPct(tR)), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: col(sR),
      textAlign: 'right',
  ...
  }, fPct(sR)), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: col(rel),
      fontWeight: 700,
  ...
  }, rel != null ? (rel >= 0 ? '↑' : '↓') + Math.abs(rel).toFixed(1) + '%' : '–')))), impliedBeta != null && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 8,
  ...
  }, "Beta impl\xEDcita vs sector (1Y)"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 12,
      fontWeight: 700,
      fontFamily: 'Geist Mono,monospace',
  ...
  }, impliedBeta.toFixed(2), "x"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 9,
      color: '#33353f'
    }
  }, impliedBeta > 1.5 ? 'alta volatilidad' : impliedBeta > 1.1 ? 'amplifica sector' : impliedBeta < 0.7 ? 'defensivo' : 'en línea con sector')), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 9,
      color: '#33353f',
      marginTop: 8,
  ...
  }, "Alpha = ticker \u2212 sector ETF \xB7 Beta impl\xEDcita = retorno ticker / retorno ETF (1Y) \xB7 datos EOD FMP"));
}

// ─── RDCF ENTRY ZONE ─────────────────────────────────────────
function RdcfEntryZoneCard({
  rdcf,
  currentPrice,
  hist
}) {
  ...
  if (prem < -30) {
    ...
    zone = 'Fair Value';
    zColor = '#eca851';
    action = 'Neutral — mantener';
  } else if (prem < 30) {
    zone = 'Premium';
    zColor = '#fca5a5';
    action = 'Reducir en rebotes';
  } else {
    zone = 'Overvalued';
    zColor = '#eb6459';
  ...
  if (prem < -10 && rsi != null && rsi < 45) {
    ...
    signal = 'INFRAVALORADO — confirmar técnicos';
    sigColor = '#86efac';
  }
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "RDCF Entry Zone"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 8,
      flexWrap: 'wrap',
  ...
  }, "Precio actual"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 15,
      fontWeight: 700,
      color: '#edeef4',
  ...
  }, f$(currentPrice))), /*#__PURE__*/React.createElement("div", {
    style: {
      background: '#15151c',
      borderRadius: 6,
      padding: '8px 12px',
  ...
  }, "Valor RDCF"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 15,
      fontWeight: 700,
      color: '#968ff7',
  ...
  }, f$(fair))), /*#__PURE__*/React.createElement("div", {
    style: {
      background: '#15151c',
      borderRadius: 6,
      padding: '8px 12px',
  ...
  }, fp(prem)), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 9,
      color: '#787a83',
      marginTop: 2
  ...
  }, rsi != null && /*#__PURE__*/React.createElement("div", {
    style: {
      background: '#15151c',
      borderRadius: 6,
      padding: '6px 10px'
    ...
    style: {
      fontSize: 11,
      fontWeight: 700,
      color: rsi < 30 ? '#5ac576' : rsi > 70 ? '#eb6459' : '#eca851',
      fontFamily: 'Geist Mono,monospace'
    }
  }, rsi.toFixed(0))), ema50 != null && /*#__PURE__*/React.createElement("div", {
    style: {
      background: '#15151c',
      borderRadius: 6,
      padding: '6px 10px'
    ...
    style: {
      fontSize: 11,
      fontWeight: 700,
      color: aboveE50 ? '#5ac576' : '#eb6459',
      fontFamily: 'Geist Mono,monospace'
    }
  }, f$(ema50), " ", aboveE50 ? '↑' : '↓')), rdcf.impliedG1 != null && /*#__PURE__*/React.createElement("div", {
    style: {
      background: '#15151c',
      borderRadius: 6,
      padding: '6px 10px'
  ...
  }, "g\u2081 impl\xEDcito (a\xF1o 1) "), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11,
      fontWeight: 700,
      color: '#968ff7',
  ...
  }, (rdcf.impliedG1 * 100).toFixed(1), "%")), rdcf.revCagr != null && /*#__PURE__*/React.createElement("div", {
    style: {
      background: '#15151c',
      borderRadius: 6,
      padding: '6px 10px'
  ...
  }, "CAGR impl\xEDcito (", rdcf.last?.horizon ?? 5, "a) "), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11,
      fontWeight: 700,
      color: '#a5b4fc',
  ...
  }, (rdcf.revCagr * 100).toFixed(1), "%"), rdcf.analystGrowth != null && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 9,
      color: '#787a83'
    }
  }, " \xB7 consenso ", (rdcf.analystGrowth * 100).toFixed(1), "%"))), /*#__PURE__*/React.createElement("div", {
    style: {
  ...
  }, signal), rdcf.lowConfidence && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 9,
      color: '#787a83',
      marginTop: 6,
  ...
  }, "\u26A0 Baja confianza (rf por defecto \u2014 sin macro_state). Usar orientativamente."), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 9,
      color: '#33353f',
      marginTop: 6,
  ...
  }, "RDCF basado en FCF proyectado y WACC. No es asesor\xEDa de inversi\xF3n."));
}
...
function EVEBITDAHistory({
  ...
  const series = useMemo(() => {
    ...
    const priceAt = dateStr => {
      ...
      let chosen = null;
      for (const h of hist) {
        if (new Date(h.date).getTime() <= t) chosen = h.close;else break;
      }
  ...
  const minV = Math.min(...vals, median),
    maxV = Math.max(...vals, median),
    rng = maxV - minV || 1;
  const W = 800,
    H = 170,
    pt = 14,
  ...
  }, /*#__PURE__*/React.createElement(SectionTitle, null, "EV/EBITDA \u2014 Hist\xF3rico (", series.length, "Q)"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 18,
      marginBottom: 10,
  ...
  }, "Mediana"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 20,
      fontWeight: 800,
      color: '#a6a7b1',
...
function CapGainsBudgetPanel({
  ...
  const [positions, setPositions] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(CG_STORE_KEY) || '[]');
    } catch {
      return [];
    }
  });
  ...
  useEffect(() => {
    if (activeTicker && sym.toUpperCase() === activeTicker.toUpperCase() && activePrice > 0 && curPx === '') setCurPx(String(activePrice.toFixed(2)));
  }, [sym, activeTicker, activePrice]);
  const save = ps => {
    setPositions(ps);
    try {
      localStorage.setItem(CG_STORE_KEY, JSON.stringify(ps));
    } catch {}
  };
  const addPos = () => {
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Capital Gains Budget Tracker"), enriched.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 8,
      flexWrap: 'wrap',
  ...
  }, val)))), enriched.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexDirection: 'column',
      gap: 3,
  ...
  }, enriched.map((e, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    style: {
      display: 'flex',
      alignItems: 'center',
  ...
  }, e.sym), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 10,
      color: '#787a83'
    }
  }, e.shares, "\xD7"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 10,
      color: '#787a83',
      flex: 1
    }
  }, "$", e.costB, " \u2192 $", e.curPx), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 10,
      fontWeight: 600,
      color: e.isLTCG ? '#5ac576' : '#eca851'
    }
  }, e.isLTCG ? 'LTCG' : 'STCG', " ", e.days, "d"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 10,
      fontWeight: 700,
      color: e.gl >= 0 ? '#5ac576' : '#eb6459',
      fontFamily: 'Geist Mono,monospace'
    }
  }, fK(e.gl)), e.gl > 0 && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 9,
      color: '#eb6459'
    }
  }, "\u2212", f$(e.tax), " tax"), e.gl < 0 && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 9,
      color: '#5ac576'
    }
  }, "TLH +", f$(e.tlh)), !e.isLTCG && e.gl > 0 && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 9,
      color: '#787a83'
    }
  }, e.daysLeft, "d\u2192LTCG"), /*#__PURE__*/React.createElement("button", {
    onClick: () => removePos(i),
    style: {
      background: 'none',
      border: 'none',
    ...
    type: "number",
    value: curPx,
    onChange: e => setCurPx(e.target.value),
    style: {
      ...inputStyle,
      width: 72
    }
  ...
  }, "Datos en localStorage del browser \xB7 tasas top bracket (LTCG 20%, STCG 37%) \xB7 no es asesor\xEDa fiscal"));
}
...
Write 2-3 crisp sentences. No bullet points. Reference specific metrics. End with the rating word (STRONG BUY / BUY / HOLD / CAUTION / AVOID).`;
  ...
  const summarizeEarnings = useCallback(async () => {
    ...
    try {
      ...
      const payload = {
        ...
        priceTarget,
        analystConsensus,
        macroRegime: macroTilt?.regime ?? null,
        macroTilt: macroTilt?.tilt ?? null,
        macroReasons: macroTilt?.reasons ?? null
      };
  ...
  const analyze = useCallback(async sym => {
    ...
    try {
      ...
      if (sEtf) {
        try {
          const sh = await fmpGet('historical-price-eod/full', {
            symbol: sEtf
          });
          if (Array.isArray(sh) && sh.length) {
            setSectorHist(sh);
            setSectorEtfSym(sEtf);
          }
        } catch {}
      }
      ...
      if (sb) {
        try {
          ...
          if (sess) {
            await sb.from('sl_analyses').insert({
              ...
              ...(SL_FLAGS.REVERSE_DCF_ENABLED ? {
                reverse_dcf: _rdcf || null
              } : {})
            });
          }
        ...
        try {
          const {
            data: histRows
          } = await sb.from('sl_analyses').select('analysis_date, score_total, macro_tilt').eq('ticker', sym.toUpperCase()).order('analysis_date', {
            ascending: true
          });
  ...
  const exportCSV = () => {
    ...
    const rows = [['StockLens Export', ticker, date], [], ['Company', prof?.companyName || ''], ['Ticker', ticker], ['Exchange', prof?.exchange || (intlMeta?.name ?? '')], ['Sector', prof?.sector || ''], ['Industry', prof?.industry || ''], ['Export Date', date], [], ['── SCORES ──'], ['Composite Score', n(scores.total), '/ 100'], ['IC Score', n(macroAdj), '/ 100'], ['Rating', rating?.label || ''], ['Macro Tilt', n(tiltN), 'pts'], ['Valuation', n(scores.val), '/ 25'], ['Financial Health', n(scores.hlth), '/ 30'], ['Momentum', n(scores.mom), '/ 25'], ['Growth', n(scores.growth), '/ 20'], [], ['── PRICE ──'], ['Price', n(priceNow)], ['DCF Fair Value', n(dcfVal)], ['Margin of Safety', ok(mosFrac) ? `${(mosFrac * 100).toFixed(1)}%` : ''], [], ['── KEY METRICS (TTM) ──'], ['P/E', n(met?.peRatioTTM ?? met?.priceToEarningsRatioTTM)], ['EV/EBITDA', n(met?.evToEBITDATTM ?? met?.enterpriseValueOverEBITDATTM)], ['P/S', n(met?.priceToSalesRatioTTM)], ['P/B', n(met?.priceToBookRatioTTM)], ['ROE', ok(met?.returnOnEquityTTM ?? met?.roeTTM) ? `${((met?.returnOnEquityTTM ?? met?.roeTTM) * 100).toFixed(1)}%` : ''], ['ROIC', ok(met?.roicTTM) ? `${(met?.roicTTM * 100).toFixed(1)}%` : ''], ['Net Margin', ok(rat?.netProfitMarginTTM) ? `${(rat?.netProfitMarginTTM * 100).toFixed(1)}%` : ''], ['Gross Margin', ok(rat?.grossProfitMarginTTM) ? `${(rat?.grossProfitMarginTTM * 100).toFixed(1)}%` : ''], ['Revenue Growth', ok(rat?.revenueGrowthTTM ?? met?.revenueGrowthTTM) ? `${((rat?.revenueGrowthTTM ?? met?.revenueGrowthTTM) * 100).toFixed(1)}%` : ''], ['Net Debt/EBITDA', n(met?.netDebtToEBITDATTM)], ['Interest Coverage', n(met?.interestCoverageTTM ?? met?.interestCoverageRatioTTM)], ['Current Ratio', n(met?.currentRatioTTM)], [], ['── ANALYST CONSENSUS ──'], ['PT Consensus', ptList?.length ? `$${(ptList.reduce((s, p) => s + (p.priceTarget || 0), 0) / ptList.length).toFixed(2)}` : ''], ['# Analysts', n(ptList?.length)], [], ['Disclaimer', 'Solo fines informativos. No es consejo de inversión. Verificar con la fuente.']];
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, prof?.ceo && /*#__PURE__*/React.createElement("span", null, "CEO: ", prof.ceo), prof?.fullTimeEmployees && /*#__PURE__*/React.createElement("span", null, "\uD83D\uDC65 ", Number(prof.fullTimeEmployees).toLocaleString(), " employees"), prof?.ipoDate && /*#__PURE__*/React.createElement("span", null, "Est. ", prof.ipoDate?.substring(0, 4)), (() => {
    const tg = ipoEventTag(prof);
    return tg ? /*#__PURE__*/React.createElement("span", {
      title: tg.note,
      style: {
        padding: '1px 7px',
        borderRadius: 20,
        fontWeight: 700,
        color: tg.color,
        background: `color-mix(in srgb, ${tg.color} 14%, transparent)`,
        border: `1px solid color-mix(in srgb, ${tg.color} 32%, transparent)`
      }
    }, "\u2691 ", tg.label) : null;
  })(), prof?.website && /*#__PURE__*/React.createElement("a", {
    href: prof.website,
    target: "_blank",
    rel: "noopener noreferrer",
    style: {
  ...
  }, adjRating?.label)), adjRating && baseRating && adjRating.label !== baseRating.label && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10,
      fontWeight: 700,
      color: tiltN > 0 ? '#5ac576' : '#eb6459'
    ...
    stmts: stmts,
    currentPrice: priceNow
  }), stmts.length >= 4 && /*#__PURE__*/React.createElement(EVEBITDAHistory, {
    stmts: stmts,
  ...
  }, "No price data")), hist.length > 0 && /*#__PURE__*/React.createElement(TLSignalPanel, {
    history: hist,
    scores: scores,
    macroTilt: macroTilt
  }), /*#__PURE__*/React.createElement(TechnicalSignals, {
    history: hist,
  ...
  }, /*#__PURE__*/React.createElement(CapGainsBudgetPanel, {
    activeTicker: ticker,
    activePrice: quote?.price
  ...
  }, reverseDcf ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(ReverseDcfCard, {
    data: reverseDcf
  }), /*#__PURE__*/React.createElement(RdcfEntryZoneCard, {
    rdcf: reverseDcf,
    ...
    holders: instHolders,
    sharesOut: quote?.sharesOutstanding ?? prof?.sharesOutstanding
  }), congressTrades.length > 0 && /*#__PURE__*/React.createElement(CongressionalTradesPanel, {
    trades: congressTrades
  })), sectorHist.length > 0 && /*#__PURE__*/React.createElement(SectorRelStrengthCard, {
    tickerHist: hist,
    sectorHist: sectorHist,
    sectorEtf: sectorEtfSym,
    sector: prof?.sector,
    ticker: ticker
    ...
    ticker: ticker,
    basis: taxBasis,
    setBasis: setTaxBasis,
    buyDate: taxBuyDate,
    setBuyDate: setTaxBuyDate
  }), /*#__PURE__*/React.createElement("div", {
    style: {
      background: '#1c1d26',
      border: '1px solid #24262f',
